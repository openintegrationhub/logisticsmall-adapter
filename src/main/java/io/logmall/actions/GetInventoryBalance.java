...
public class GetInventoryBalance implements Module {
	...
	public void execute(final ExecutionParameters parameters) {
		...
		try {
			...
			ParametersJsonMapper<BalanceParameters> parametersJsonMapper = new ParametersJsonMapper<>(
					BalanceParameters.class);
			if (parameters.getMessage().getBody() != null) {
				BalanceParameters balanceParameters = parametersJsonMapper.fromJson(parameters.getMessage().getBody());
				// do somethin 
			}
	...
	private JsonObject getEventBody(ShowInventoryBalance resultBod) throws JAXBException {
		...
		ParametersJsonMapper<BalanceMinimal> mapper = new ParametersJsonMapper<>(BalanceMinimal.class);
